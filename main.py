...
from src.config import EJARN_LIST_URL, EJARN_MAX_ARTICLES, EJARN_LOGIN_EMAIL, EJARN_LOGIN_PASSWORD
...
def main() -> None:
    parser = argparse.ArgumentParser(description="eJARN 최신 기사 수집 후 Pydantic 구조로 정규화")
    parser.add_argument("--max", type=int, default=EJARN_MAX_ARTICLES, help="수집 기사 수 상한 (기본: 설정값)")
    parser.add_argument("--output", "-o", type=str, default="", help="결과 저장 경로 (JSON). 비우면 stdout")
    parser.add_argument("--no-verify", action="store_true", help="SSL 검증 비활성화")
    parser.add_argument("--list-url", type=str, default="", help="목록 페이지 URL (지정 시 메뉴 선택 생략)")
    parser.add_argument("--agent", action="store_true", help="LangChain 에이전트로 실행 (System/Human/AI/Tool 흐름)")
    parser.add_argument(
    ...
    if args.agent:
        print("수집 실패: --agent 모드는 HITL 로그인 강제 정책과 호환되지 않습니다. --agent 없이 실행하세요.", file=sys.stderr)
        sys.exit(1)
    if not email or not password:
        print("수집 실패: .env의 EJARN_LOGIN_EMAIL / EJARN_LOGIN_PASSWORD가 필요합니다.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        if args.publication_jarn_regular:
            ...
            collection = run_publication_jarn_regular(max_articles=5, require_hitl_login=True)
            output_path = args.output or "publication_jarn_regular.json"
        else:
            from src.pipeline import run_pipeline, run_jarn_regular_balanced, run_jarn_special_balanced
